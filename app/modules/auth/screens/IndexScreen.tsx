  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      try {
        ...
        setBackgroundImages(
          posterPaths.map((path) => `https://image.tmdb.org/t/p/w500${path}`)
        );
      } catch (error) {
    ...
    <ImageBackground
      source={{
        uri:
          ...
          "ruta/a/una/imagen/predeterminada.jpg",
      }}
      style={styles.background}
    >
      <View
        style={[tw`flex-1 justify-center items-center p-4`, styles.overlay]}
      >
        <Animated.View style={{ opacity: fadeAnim, alignItems: "center" }}>
          <Text style={[tw`text-4xl font-bold mb-2`, styles.title]}>
            CineScore
          </Text>
          ...
          <TouchableOpacity
            style={[
              tw`flex-row items-center justify-center p-4 rounded-lg mt-6 w-64`,
              styles.loginButton,
            ]}
            onPress={async () => {
              try {
                await router.push("/modules/auth/screens/LoginScreen");
              } catch (error) {
          ...
          >
            ...
            <Text style={[tw`text-lg text-center`, styles.buttonText]}>
              Iniciar Sesión
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              tw`flex-row items-center justify-center p-4 rounded-lg mt-4 w-64`,
              styles.registerButton,
            ]}
            onPress={async () => {
              try {
                await router.push("/modules/auth/screens/RegisterScreen");
              } catch (error) {
